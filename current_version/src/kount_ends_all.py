...
def kount_kmer(seq,kmer_array,k):
...
def main():
    ...
    while line!='':
        ...
        if line=="":
            #kmer_array=init_kmer_array(kmer)
            seq_ends=seq[0:10]+seq[-10:]
            k_ends=kount_kmer(seq_ends,kmer_array,kmer,label)
            k_all=kount_kmer(seq,kmer_array,kmer,label)
            k_features=np.append(label,k_all,k_ends)
            k_features.tofile(output,sep=":",format="%s")
